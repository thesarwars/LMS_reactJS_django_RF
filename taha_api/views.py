    ...
    def get_queryset(self):
        ...
        #  recommended course function
            ...
            queries = [Q(techs__iendswith=value)
                       for value in student.intereseted_cat]
            query = queries.pop()
            for item in queries:
                query |= item
...
def enroll_status(request, student_id, course_id):
    ...
    enrollStatus = EnrollCourseStudent.objects.filter(
    ...
    def get_queryset(self):
        ...
        if 'popular' in self.request.GET:
            queryset = queryset.annotate(
                avg_rating = Coalesce(Avg('courserating__rating'), 0),
                num_ratings = Count('courserating')
            ).order_by('-avg_rating', '-num_rating')[:4]
        return queryset
        # if 'popular' in self.request.GET:
        #     sql = "SELECT c.id, c.name, AVG(cr.rating) as avg_rating FROM main_courserating as cr INNER JOIN main_course as c ON cr.course_id=c.id GROUP BY c.id ORDER BY avg_rating desc LIMIT 4"
...
def rating_status(request, student_id, course_id):
    student = Student.objects.filter(id=student_id).first()
    course = Course.objects.filter(id=course_id).first()
    enrollStatus = CourseRating.objects.filter(
        course=course, student=student).count()
...
def favourite_status(request, student_id, course_id):
    student = Student.objects.filter(id=student_id).first()
    course = Course.objects.filter(id=course_id).first()
    favouriteStatus = AddToFav.objects.filter(
...
class MyAssignmentView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        Notification.objects.filter(
            student=student, notif_to='students', notif_sub='assignment').update(is_read=True)
...
def quiz_attempt_status(request, quiz_id, student_id):
    quiz = Quiz.objects.filter(id=quiz_id).first()
    student = Student.objects.filter(id=student_id).first()
    attemptStatus = AttemptQuiz.objects.filter(
